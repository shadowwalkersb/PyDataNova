...
@router.get("/")
async def get_items(db: str = Query(..., description="DB: elephant | supra | mongo")):
    logger.info(f"Fetching all items from {db}")
    return items_service.get_all_items(db)

@router.get("/{item_id}")
async def get_item(item_id: int, db: str = Query(...)):
...
@router.post("/")
async def create_item(payload: dict, db: str = Query(...)):
    try:
